<script setup>
    ...
    const getDataReport = ()=>{
        ...
        ReporteController.getAlarmasPorDispositivos(filters.value).then(data =>{
            const labels = data.map(item => item.Nombre);
            const values = data.map(item => item.CantidadAlarmas);
            $appStore.setGlobalLoading(false);
            emit('onReport', {
                title: 'Alarmas por dispositivo',
                labels,
                values
            });
...
<template>
   ...
   <BaseCard class="flex-1 text-center" :hover="true" @click="showModal = true">
        ...
        <h3>Cantidad de Alarmas por Dispositivos</h3>
    </BaseCard>
